...
author: "Andrés Gordo Ortiz (PSancho Lab)"
...
diffLipids_mcm_n215 <- lipid_matrix_norm$E[results[,1] !=0,]
diffLipids.df_mcm_n215 <- as_tibble(diffLipids_mcm_n215, rownames = "Lipid_Names")

resultsDEL<-myTopHits.df[myTopHits.df$Lipid_Name %in% diffLipids.df_mcm_n215$Lipid_Names,]
...
results <- decideTests(ebFit, method="global", adjust.method="BH", p.value=0.05, lfc=1)
colnames(lipid_matrix_norm$E) <- sampleLabels
diffLipids_eto_n215 <- lipid_matrix_norm$E[results[,1] !=0,]
diffLipids.df_eto_n215 <- as_tibble(diffLipids_eto_n215, rownames = "Lipid_Names")

resultsDEL<-myTopHits.df[myTopHits.df$Lipid_Name %in% diffLipids.df_eto_n215$Lipid_Names,]
...
results <- decideTests(ebFit, method="global", adjust.method="BH", p.value=0.05, lfc=1)
colnames(lipid_matrix_norm$E) <- sampleLabels
diffLipids_mcm_n253 <- lipid_matrix_norm$E[results[,1] !=0,]
diffLipids.df_mcm_n253 <- as_tibble(diffLipids_mcm_n253, rownames = "Lipid_Names")

resultsDEL<-myTopHits.df[myTopHits.df$Lipid_Name %in% diffLipids.df_mcm_n253$Lipid_Names,]
...
results <- decideTests(ebFit, method="global", adjust.method="BH", p.value=0.05, lfc=1)
colnames(lipid_matrix_norm$E) <- sampleLabels
diffLipids_eto_n253 <- lipid_matrix_norm$E[results[,1] !=0,]
diffLipids.df_eto_n253 <- as_tibble(diffLipids_eto_n253, rownames = "Lipid_Names")

resultsDEL<-myTopHits.df[myTopHits.df$Lipid_Name %in% diffLipids.df_eto_n253$Lipid_Names,]
...
results <- decideTests(ebFit, method="global", adjust.method="BH", p.value=0.05, lfc=1)
colnames(lipid_matrix_norm$E) <- sampleLabels
diffLipids_mcm_all <- lipid_matrix_norm$E[results[,1] !=0,]
diffLipids.df_mcm_all <- as_tibble(diffLipids_mcm_all, rownames = "Lipid_Names")

resultsDEL<-myTopHits.df[myTopHits.df$Lipid_Name %in% diffLipids.df_mcm_all$Lipid_Names,]
...
results <- decideTests(ebFit, method="global", adjust.method="BH", p.value=0.05, lfc=1)
colnames(lipid_matrix_norm$E) <- sampleLabels
diffLipids_eto_all <- lipid_matrix_norm$E[results[,1] !=0,]
diffLipids.df_eto_all <- as_tibble(diffLipids_eto_all, rownames = "Lipid_Names")

resultsDEL<-myTopHits.df[myTopHits.df$Lipid_Name %in% diffLipids.df_eto_all$Lipid_Names,]
...
```{r venn eto tibble}
# Obtain all the DEL which are in common
common_eto<-Reduce(intersect, list(DEL_N215_eto, DEL_N253_eto, DEL_all_eto))
# Output a table with the list of comon_eto
common_eto.df<-as_tibble(common_eto)
write.csv(common_eto.df, "data/venn_intersection_DEL_eto.csv", row.names = FALSE)
datatable(common_eto.df, 
          extensions = c('KeyTable', "FixedHeader"),
          caption="Common Differentially Expressed Lipids in Eto vs Control",
          options = list(keys = TRUE, 
                         searchHighlight = TRUE, 
                         pageLength = 10, 
                         lengthMenu = c("10", "25", "50", "100")))
...
venn_mcm
# Obtain all the DEL which are in common
common_mcm<-Reduce(intersect, list(DEL_N215_mcm, DEL_N253_mcm, DEL_all_mcm))

```

### Eto & MCM in N215
```{r venn eto mcm n215}
venn_eto_mcm_n215 <- ggvenn(data = list(N215_eto = DEL_N215_eto, N215_mcm = DEL_N215_mcm))
venn_eto
```

```{r common eto mcm in N215}
common_eto_mcm_n215<-DEL_N215_eto[DEL_N215_eto %in% DEL_N215_mcm]
print(paste0("Common Lipids between MCM and Eto in N215 are ",paste(common_eto_mcm_n215, collapse=" ") ))

```

### Eto & MCM in N253
```{r venn eto mcm n253}
venn_eto_mcm_n253 <- ggvenn(data = list(N253_eto = DEL_N253_eto, N253_mcm = DEL_N253_mcm))
venn_eto
```

```{r common eto mcm in N253}
common_eto_mcm_n253<-DEL_N253_eto[DEL_N253_eto %in% DEL_N253_mcm]
print(paste0("Common Lipids between MCM and Eto in N253 are ",paste(common_eto_mcm_n253, collapse=" ") ))
...
## Eto vs Control All Cell lines

```{r heatmap DEL eto all, results='hide'}
myheatcolors <- rev(brewer.pal(name="RdBu", n=11))
clustRows <- hclust(as.dist(1-cor(t(diffLipids_eto_all), method="pearson")), method="complete") 

clustColumns <- hclust(as.dist(1-cor(diffLipids_eto_all, method="spearman")), method="complete")
...
svg(file = "plots/heatmap_DEL_eto_vs_control_alllines.svg")
heatmap.2(diffLipids_eto_all, 
          ...
          main = "Lipid Differences Eto vs Control All Lines",  # Add a main title
...
png(file = "plots/heatmap_DEL_eto_vs_control_alllines.png")
heatmap.2(diffLipids_eto_all, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
...
```{r heatmaply DELs eto all}

heatmaply(diffLipids_eto_all, 
          colors = myheatcolors,
          Rowv=as.dendrogram(clustRows),
          RowSideColors=module.color,
          ColSideColors = module.color_col,
          ...
          scale='row',
          main ="Lipid Differences Eto vs Control All Lines")

```

## MCM vs Control All Cell Lines

```{r heatmap DEL mcm all, results='hide'}
myheatcolors <- rev(brewer.pal(name="RdBu", n=11))
clustRows <- hclust(as.dist(1-cor(t(diffLipids_mcm_all), method="pearson")), method="complete") 

clustColumns <- hclust(as.dist(1-cor(diffLipids_mcm_all, method="spearman")), method="complete")
module.assign <- cutree(clustRows, k=2) #Important parametre
module.color <- rainbow(length(unique(module.assign)), start=0.1, end=0.9) 
module.color <- module.color[as.vector(module.assign)] 
...
svg(file = "plots/heatmap_DEL_mcm_vs_control_alllines.svg")
heatmap.2(diffLipids_mcm_all, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
          ColSideColors = module.color_col,
          ...
          key.ylab = NULL,  # Remove color key y-axis label
          key.xlab = "Z-Score",  # Add x-axis label to the color key
          main = "Lipid Differences MCM vs Control All Lines",  # Add a main title
          xlab = "Samples",  # Label for x-axis
          ylab = "Lipids",  # Label for y-axis
          mar = c(5, 4),  # Adjust plot margins
          srtCol = 45,  # Rotate column labels for better readability
...
png(file = "plots/heatmap_DEL_mcm_vs_control_alllines.png")
heatmap.2(diffLipids_mcm_all, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
...
```{r heatmaply DELs mcm all}

heatmaply(diffLipids_mcm_all, 
          colors = myheatcolors,
          Rowv=as.dendrogram(clustRows),
          RowSideColors=module.color,
          ColSideColors = module.color_col,
          showticklabels=c(TRUE,FALSE),
          scale='row',
          main ="Lipid Differences MCM vs Control All Lines")

```

## Eto vs Control N215

```{r heatmap DEL eto n215, results='hide'}
myheatcolors <- rev(brewer.pal(name="RdBu", n=11))
clustRows <- hclust(as.dist(1-cor(t(diffLipids_eto_n215), method="pearson")), method="complete") 

clustColumns <- hclust(as.dist(1-cor(diffLipids_eto_n215, method="spearman")), method="complete")
module.assign <- cutree(clustRows, k=2) #Important parametre
module.color <- rainbow(length(unique(module.assign)), start=0.1, end=0.9) 
module.color <- module.color[as.vector(module.assign)] 
...
svg(file = "plots/heatmap_DEL_eto_vs_control_n215.svg")
heatmap.2(diffLipids_eto_n215, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
          ColSideColors = module.color_col,
          ...
          key.ylab = NULL,  # Remove color key y-axis label
          key.xlab = "Z-Score",  # Add x-axis label to the color key
          main = "Lipid Differences Eto vs Control N215",  # Add a main title
          xlab = "Samples",  # Label for x-axis
          ylab = "Lipids",  # Label for y-axis
          mar = c(5, 4),  # Adjust plot margins
          srtCol = 45,  # Rotate column labels for better readability
...
png(file = "plots/heatmap_DEL_eto_vs_control_n215.png")
heatmap.2(diffLipids_eto_n215, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
...
```{r heatmaply DELs eto n215}

heatmaply(diffLipids_eto_n215, 
          colors = myheatcolors,
          Rowv=as.dendrogram(clustRows),
          RowSideColors=module.color,
          ColSideColors = module.color_col,
          showticklabels=c(TRUE,FALSE),
          scale='row',
          main ="Lipid Differences Eto vs Control N215")

```

## MCM vs Control N215

```{r heatmap DEL mcm n215, results='hide'}
myheatcolors <- rev(brewer.pal(name="RdBu", n=11))
clustRows <- hclust(as.dist(1-cor(t(diffLipids_mcm_n215), method="pearson")), method="complete") 

clustColumns <- hclust(as.dist(1-cor(diffLipids_mcm_n215, method="spearman")), method="complete")
module.assign <- cutree(clustRows, k=2) #Important parametre
module.color <- rainbow(length(unique(module.assign)), start=0.1, end=0.9) 
module.color <- module.color[as.vector(module.assign)] 
...
svg(file = "plots/heatmap_DEL_mcm_vs_control_n215.svg")
heatmap.2(diffLipids_mcm_n215, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
          ColSideColors = module.color_col,
          ...
          key.ylab = NULL,  # Remove color key y-axis label
          key.xlab = "Z-Score",  # Add x-axis label to the color key
          main = "Lipid Differences MCM vs Control N215",  # Add a main title
          xlab = "Samples",  # Label for x-axis
          ylab = "Lipids",  # Label for y-axis
          mar = c(5, 4),  # Adjust plot margins
          srtCol = 45,  # Rotate column labels for better readability
...
png(file = "plots/heatmap_DEL_mcm_vs_control_n215.png")
heatmap.2(diffLipids_mcm_215, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
          ColSideColors = module.color_col,
...
```{r heatmaply DELs mcm n215}

heatmaply(diffLipids_mcm_n215, 
          colors = myheatcolors,
          Rowv=as.dendrogram(clustRows),
          RowSideColors=module.color,
          ColSideColors = module.color_col,
...
```{r heatmap DEL eto n253, results='hide'}
myheatcolors <- rev(brewer.pal(name="RdBu", n=11))
clustRows <- hclust(as.dist(1-cor(t(diffLipids_eto_n253), method="pearson")), method="complete") 

clustColumns <- hclust(as.dist(1-cor(diffLipids_eto_n253, method="spearman")), method="complete")
module.assign <- cutree(clustRows, k=2) #Important parametre
module.color <- rainbow(length(unique(module.assign)), start=0.1, end=0.9) 
module.color <- module.color[as.vector(module.assign)] 
...
svg(file = "plots/heatmap_DEL_eto_vs_control_n253.svg")
heatmap.2(diffLipids_eto_n253, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
          ColSideColors = module.color_col,
          ...
          key.ylab = NULL,  # Remove color key y-axis label
          key.xlab = "Z-Score",  # Add x-axis label to the color key
          main = "Lipid Differences Eto vs Control N253",  # Add a main title
          xlab = "Samples",  # Label for x-axis
          ylab = "Lipids",  # Label for y-axis
          mar = c(5, 4),  # Adjust plot margins
          srtCol = 45,  # Rotate column labels for better readability
...
png(file = "plots/heatmap_DEL_eto_vs_control_n253.png")
heatmap.2(diffLipids_eto_253, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
          ColSideColors = module.color_col,
...
```{r heatmaply DELs eto n253}

heatmaply(diffLipids_eto_n253, 
          colors = myheatcolors,
          Rowv=as.dendrogram(clustRows),
          RowSideColors=module.color,
          ColSideColors = module.color_col,
          showticklabels=c(TRUE,FALSE),
          scale='row',
          main ="Lipid Differences Eto vs Control N253")

```

## MCM vs Control N253

```{r heatmap DEL mcm n253, results='hide'}
myheatcolors <- rev(brewer.pal(name="RdBu", n=11))
clustRows <- hclust(as.dist(1-cor(t(diffLipids_mcm_n253), method="pearson")), method="complete") 

clustColumns <- hclust(as.dist(1-cor(diffLipids_mcm_n253, method="spearman")), method="complete")
module.assign <- cutree(clustRows, k=2) #Important parametre
module.color <- rainbow(length(unique(module.assign)), start=0.1, end=0.9) 
module.color <- module.color[as.vector(module.assign)] 
...
svg(file = "plots/heatmap_DEL_mcm_vs_control_n253.svg")
heatmap.2(diffLipids_mcm_n253, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
          ColSideColors = module.color_col,
          ...
          key.ylab = NULL,  # Remove color key y-axis label
          key.xlab = "Z-Score",  # Add x-axis label to the color key
          main = "Lipid Differences MCM vs Control N253",  # Add a main title
          xlab = "Samples",  # Label for x-axis
          ylab = "Lipids",  # Label for y-axis
          mar = c(5, 4),  # Adjust plot margins
          srtCol = 45,  # Rotate column labels for better readability
...
png(file = "plots/heatmap_DEL_mcm_vs_control_n253.png")
heatmap.2(diffLipids_mcm_253, 
          Rowv = as.dendrogram(clustRows), 
          Colv = as.dendrogram(clustColumns),
          RowSideColors = module.color,
          ColSideColors = module.color_col,
...
```{r heatmaply DELs mcm n253}

heatmaply(diffLipids_mcm_n253, 
          colors = myheatcolors,
          Rowv=as.dendrogram(clustRows),
          RowSideColors=module.color,
          ColSideColors = module.color_col,
...
## All Lipids (regardless of stats success)
