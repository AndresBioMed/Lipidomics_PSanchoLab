...
## Top Lipids by Loading
```{r top loadings lipidr}
...
top_loadings_plots +
  theme_Publication() + 
  labs(title = "Loadings/Influence of each Lipid in OPLS-DA Analysis") +
  guides(alpha = "none")
save_plot("plots/top_loadings.png",top_loadings_plots +
  theme_Publication() + 
  labs(title = "Loadings/Influence of each Lipid in OPLS-DA Analysis") +
  guides(alpha = "none"), base_width = 10, base_height = 7)
top<-top_lipids(mvaresults, top.n=100)
top_molecules<-top$Molecule
write.csv(top,"data/top_loadings_pca.csv", row.names = FALSE)
```

```{r top lipids}
datatable(top[, c(2,7,8)], 
          extensions = c('KeyTable', "FixedHeader"), 
          caption = "Top 100 Lipids by Loading Relevance",
...
An **important** note: all the contrasts, design matrix and differential expression analysis are made according to the formula **condition1- condition2**. This means that a *logfold change* > 0 shows that the first condition is over represented, whereas a *logfold change* < 0 shows the contrary.

## MCM vs Control
```{r limma and Vplot mcm vs control}
condition <- colData(d_invitro)$treatment
condition <- factor(condition)
sampleLabels<-rownames(colData(d_invitro))
...
write.csv(myTopHits.df, "data/stats_mcm_vs_control.csv", row.names = FALSE)
...
vplot <- ggplot(myTopHits.df) +
  ...
  labs(title="Volcano plot [MCM vs Control]",
       caption=paste0("produced on ", Sys.time())) +
  scale_x_continuous(n.breaks = 8)+
  theme_Publication()

save_plot("plots/volcano_mcm_vs_control.svg",vplot, base_width = 15, base_height = 7)
save_plot("plots/volcano_mcm_vs_control.png",vplot, base_width = 15, base_height = 7)
...
write.csv(resultsDEL, "data/diffLipids_mcm_vs_control.csv", row.names = FALSE)

```

```{r table DEL mcm vs control}
datatable(resultsDEL, 
          extensions = c('KeyTable', "FixedHeader"),
          caption="Diferentially Expressed Lipids in MCM vs Control",
...
```
***As it can be seen, no Lipid was found to be differentially represented (FDR<=0.05, LogFC>=1) between MCM and Control samples, which makes sense according to the PCA Plots results.***

## Eto vs Control
```{r limma and Vplot etoc vs control}
...
write.csv(myTopHits.df, "data/stats_eto_vs_control.csv", row.names = FALSE)
...
vplot <- ggplot(myTopHits.df) +
  ...
  #annotate("rect", xmin = 1, xmax = 12, ymin = -log10(0.01), ymax = 7.5, alpha=.2, fill="#BE684D") +
  #annotate("rect", xmin = -1, xmax = -12, ymin = -log10(0.01), ymax = 7.5, alpha=.2, fill="#2C467A") +
  labs(title="Volcano plot [Eto vs Control]",
       caption=paste0("produced on ", Sys.time())) +
  scale_x_continuous(n.breaks = 8)+
  theme_Publication()

save_plot("plots/volcano_eto_vs_control.svg",vplot, base_width = 15, base_height = 7)
save_plot("plots/volcano_eto_vs_control.png",vplot, base_width = 15, base_height = 7)
...
resultsDEL<-myTopHits.df[myTopHits.df$Lipid_Name %in% diffLipids.df$Lipid_Names,]
write.csv(resultsDEL, "data/diffLipids_eto_vs_control.csv", row.names = FALSE)

```

```{r table DEL eto vs control}
datatable(resultsDEL, 
          extensions = c('KeyTable', "FixedHeader"),
          caption="Differentially Expressed Lipids in Eto vs Control",
          options = list(keys = TRUE, 
                         searchHighlight = TRUE, 
                         pageLength = 10, 
                         lengthMenu = c("10", "25", "50", "100")))
